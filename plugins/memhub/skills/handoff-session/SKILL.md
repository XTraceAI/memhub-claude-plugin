---
description: Use when the user wants to hand off the current session/work to a teammate via MemHub (e.g. "hand this off to Alice", "handoff this session to Bob", "share my context with Carol so she can pick this up", "pass this work to X"). Creates a shareable context base holding a handoff brief plus the full session's extracted memory, and shares it read-only with the teammate.
argument-hint: <teammate> [title...]
allowed-tools: mcp__memhub-staging__list_teammates, mcp__memhub-staging__create_context_base, mcp__memhub-staging__save_artifact, mcp__memhub-staging__share_context_base, Bash
---

Hand the current session off to a teammate: bundle a concise handoff brief and
the session's full extracted memory into one context base and share it
read-only. The teammate's agent then picks it up by searching that context
base — no transcript pasting, no shoulder-tap walkthrough.
...
2. Create the handoff container: `create_context_base` with
...
3. Write the handoff brief and save it with `save_artifact` into that context
   base (`context_base_id` from step 2, `artifact_type: "document"`,
...
4. Share it: `share_context_base` with the context base id and the teammate's
   `user_id`. Read-only is what you get and all a handoff needs.

5. Ship the full session into the same context base via Bash — one command:

   ```bash
   uv run --with mcp python "${CLAUDE_PLUGIN_ROOT}/scripts/import_session.py" \
     --session "<current-session-id-or-path>" \
     --context-base-id "<id-from-step-2>" \
...
6. Report back: the context base name, who it's shared with, and the
   receiving line the user can send their teammate verbatim — e.g.:

   > Ask your agent: *search the "Handoff: <title>" context base in memhub*
...
If `share_context_base` fails on permissions, you don't have contributor
access to the context base — this happens when reusing someone else's context
base instead of creating one in step 2; create your own and retry.
